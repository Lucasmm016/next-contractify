...
} : unknown) & (T['body'] extends z.ZodType ? {
    body: z.infer<T['body']>;
} : unknown) & (T['params'] extends z.ZodType ? {
    params: z.infer<T['params']>;
} : unknown);
export type InferSuccessResponse<T extends RouteDefinition> = T['response'] extends {
    success: z.ZodType;
} ? z.infer<T['response']['success']> : unknown;
export type InferErrorResponse<T extends RouteDefinition> = T['response'] extends {
    error: z.ZodType;
} ? z.infer<T['response']['error']> : unknown;
export type InferResponse<T extends RouteDefinition> = InferSuccessResponse<T> | InferErrorResponse<T>;
